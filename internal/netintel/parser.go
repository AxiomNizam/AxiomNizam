...
func (pe *ParserEngine) registerLogTypes() {
	pe.logTypes = []LogTypeInfo{
		{ID: LogWiFiProbe, Name: "Wi-Fi Probe Requests", Description: "802.11 probe requests from devices scanning for networks", Category: "wireless", Fields: []string{"device_mac", "ssid", "signal_dbm", "channel", "vendor"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-wifi"><path d="M12 20h.01"/><path d="M2 8.82a15 15 0 0 1 20 0"/><path d="M5 12.86a10 10 0 0 1 14 0"/><path d="M8.5 16.43a5 5 0 0 1 7 0"/></svg>`},
		{ID: LogIPFlow, Name: "IP Flow Records", Description: "Aggregated network flow data (NetFlow/sFlow/IPFIX format)", Category: "network", Fields: []string{"src_ip", "dst_ip", "src_port", "dst_port", "protocol", "bytes", "packets"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-flow"><path d="M3 3v18h18"/><path d="m7 16 4-8 4 5 4-9"/><circle cx="7" cy="16" r="1.5" fill="currentColor"/><circle cx="11" cy="8" r="1.5" fill="currentColor"/><circle cx="15" cy="13" r="1.5" fill="currentColor"/><circle cx="19" cy="4" r="1.5" fill="currentColor"/></svg>`},
		{ID: LogSyslog, Name: "Syslog Messages", Description: "System log messages from network devices and servers", Category: "infrastructure", Fields: []string{"facility", "severity", "hostname", "process", "message"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-syslog"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M7 8h10"/><path d="M7 12h7"/><path d="M7 16h10"/><circle cx="17" cy="12" r="1" fill="currentColor"/></svg>`},
		{ID: LogPcap, Name: "Packet Captures", Description: "Raw packet capture data in pcap/pcapng format", Category: "network", Fields: []string{"src_mac", "dst_mac", "ethertype", "protocol", "payload_size"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-pcap"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M6 8h4"/><path d="M6 12h8"/><path d="M6 16h6"/><path d="M16 8v8"/><path d="M19 8v8"/></svg>`},
		{ID: LogCSI, Name: "Channel State Information", Description: "WiFi CSI data for signal analysis and movement detection", Category: "wireless", Fields: []string{"subcarrier_count", "amplitude", "phase", "rssi", "antenna_count"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-csi"><path d="M2 12c2-3 4 3 6 0s4 3 6 0 4 3 6 0"/><path d="M2 8c2-2 4 2 6 0s4 2 6 0 4 2 6 0" opacity="0.5"/><path d="M2 16c2-2 4 2 6 0s4 2 6 0 4 2 6 0" opacity="0.5"/></svg>`},
		{ID: LogNetFlow, Name: "NetFlow/IPFIX", Description: "Cisco NetFlow or IETF IPFIX flow export records", Category: "network", Fields: []string{"src_ip", "dst_ip", "bytes", "packets", "duration", "tos"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-netflow"><circle cx="5" cy="12" r="2"/><circle cx="19" cy="6" r="2"/><circle cx="19" cy="18" r="2"/><path d="M7 11l10-4"/><path d="M7 13l10 4"/><path d="M17 6h-4"/><path d="M17 18h-4"/></svg>`},
		{ID: LogDHCP, Name: "DHCP Leases", Description: "DHCP lease events and address assignments", Category: "infrastructure", Fields: []string{"mac", "ip", "hostname", "lease_time", "action"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-dhcp"><rect x="3" y="6" width="18" height="12" rx="2"/><path d="M7 10v4"/><path d="M7 12h4"/><path d="M11 10v4"/><path d="M14 10h3v2h-3v2h3"/></svg>`},
		{ID: LogDNS, Name: "DNS Queries", Description: "DNS query and response logs", Category: "network", Fields: []string{"query", "type", "response", "client_ip", "latency_ms"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-dns"><circle cx="12" cy="12" r="9"/><path d="M12 3c-3 3-3 6 0 9s3 6 0 9"/><path d="M3 12h18"/><path d="M5.5 7h13"/><path d="M5.5 17h13"/></svg>`},
		{ID: LogRadius, Name: "RADIUS Auth", Description: "802.1X/RADIUS authentication logs", Category: "security", Fields: []string{"username", "mac", "nas_ip", "result", "reason"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-radius"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="4"/><circle cx="12" cy="12" r="1" fill="currentColor"/><path d="M12 3v4"/><path d="M12 17v4"/><path d="M3 12h4"/><path d="M17 12h4"/></svg>`},
		{ID: LogSNMP, Name: "SNMP Traps", Description: "SNMP trap and inform notifications from network devices", Category: "infrastructure", Fields: []string{"oid", "community", "agent_ip", "trap_type", "value"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-snmp"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8"/></svg>`},
		{ID: LogFirewall, Name: "Firewall Logs", Description: "Firewall accept/deny/drop action logs", Category: "security", Fields: []string{"action", "src_ip", "dst_ip", "src_port", "dst_port", "rule_id", "protocol"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-firewall"><path d="M12 2l8 4v6c0 5.5-3.8 8.2-8 10-4.2-1.8-8-4.5-8-10V6l8-4z"/><path d="M9 12l2 2 4-4"/></svg>`},
		{ID: LogBluetooth, Name: "Bluetooth LE", Description: "Bluetooth Low Energy advertisement and scan data", Category: "wireless", Fields: []string{"device_mac", "rssi", "uuid", "major", "minor", "tx_power"}, Icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="ax-icon ax-icon-bluetooth"><path d="M7 7l10 10-5 5V2l5 5L7 17"/></svg>`},
	}
}
