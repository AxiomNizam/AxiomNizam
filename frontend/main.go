...
func main() {
	...
	// Auto-detect TLS: when TLS_ENABLED=true, default backend URLs to https://
	tlsEnabled := strings.EqualFold(strings.TrimSpace(os.Getenv("TLS_ENABLED")), "true")
	tlsAutoGenerated := strings.EqualFold(strings.TrimSpace(os.Getenv("TLS_AUTO_GENERATE")), "true")
	defaultScheme := "http://"
	if tlsEnabled {
		defaultScheme = "https://"
	}

	// Create TLS-aware HTTP client for server-side fetches.
	// When using self-signed certs (dev mode), skip certificate verification.
	if tlsEnabled && tlsAutoGenerated {
		transport := &http.Transport{
			TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, //nolint:gosec // Self-signed dev certs
		}
		tlsHTTPClient = &http.Client{Timeout: 5 * time.Second, Transport: transport}
	} else {
		tlsHTTPClient = &http.Client{Timeout: 5 * time.Second}
	}

	// Get browser-facing backend URL from environment or use default.
	backendURL = trimTrailingSlash(os.Getenv("BACKEND_URL"))
	if backendURL == "" {
		backendURL = defaultScheme + "127.0.0.1:8000"
	} else if tlsEnabled && strings.HasPrefix(backendURL, "http://") {
		// Auto-upgrade to HTTPS when TLS is enabled
		backendURL = "https://" + backendURL[len("http://"):]
	}
	...
	} else if tlsEnabled && strings.HasPrefix(backendProxyURL, "http://") {
		backendProxyURL = "https://" + backendProxyURL[len("http://"):]
	}
	...
	fmt.Printf("\n🌐 Frontend Server running on %s://localhost:%s\n", scheme, port)
	...
	if tlsEnabled {
		...
		// Fall back to auto-generated certs from backend (data/certs/)
		if certFile == "" || keyFile == "" {
			if tlsAutoGenerated {
				certFile = "/data/certs/server.crt"
				keyFile = "/data/certs/server.key"
				// If shared certs from backend don't exist yet, generate our own
				if _, err := os.Stat(certFile); os.IsNotExist(err) {
					if genErr := generateSelfSignedCert(certFile, keyFile); genErr != nil {
						log.Printf("⚠️  Failed to generate TLS certs: %v — falling back to HTTP", genErr)
						certFile = ""
						keyFile = ""
					}
				}
		...
		if certFile != "" && keyFile != "" {
			log.Printf("🔒 Frontend TLS enabled on %s", addr)
			if err := http.ListenAndServeTLS(addr, certFile, keyFile, router); err != nil {
				log.Fatalf("Frontend TLS server error: %v", err)
			}
		} else {
			log.Println("⚠️  TLS_ENABLED=true but TLS_CERT_FILE/TLS_KEY_FILE not set — running on HTTP")
			router.Run(addr)
		}
	} else {
		router.Run(addr)
...
func fetchStatus() (*StatusResponse, error) {
	resp, err := tlsHTTPClient.Get(fmt.Sprintf("%s/status", backendProxyURL))
	if err != nil {
		return nil, err
	}
...
// for development when shared certs from the backend are not available.
func generateSelfSignedCert(certFile, keyFile string) error {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"AxiomNizam Frontend Dev"},
	...
	log.Printf("🔒 Generated self-signed TLS certs at %s", certsDir)
	return nil
}
